...
'http://jenkins:jenkins@gogs-user2-gogs.apps.advdev.openshift.opentlc.com/rlichti/kitchensink.git'
        ...
        sh 'mvn -s .openshift/nexus_settings.xml -P openshift
build-helper:parse-version versions:set -DbuildNumber=$BUILD_NUMBER
\'-DnewVersion=${parsedVersion.majorVersion}.${parsedVersion.minorVersion}-${buildNumber}\''
        def pom = readMavenPom file: 'pom.xml'
        
        
        
        sh 'git checkout -b build-' + pom.version
        sh 'git config user.email jenkins@example.opentlc.com && git config
user.name jenkins'
...
-DskipITs=true -DskipUTs=true clean package"
    }
...
//        sonar:sonar"
    ...
    stage('Publish') {
        ...
        sh "mvn -s .openshift/nexus_settings.xml -P openshift -DskipTests=true
-DskipITs=true -DskipUTs=true
-DaltDeploymentRepository=nexus::default::http://nexus3-user2-nexus.apps.advdev.openshift.opentlc.com/repository/maven-releases
deploy"
    }
